...
3. Generated Cypress specs contain `it.skip` / `this.skip()` for `dispute`, `dispute-evidence`, `refund`, `tenant-fee-config`, `webhook-delivery-attempt` (JHipster skips create-flows for entities with required relationships). They are generator output, are not run by `test`/`verify`, and cannot be edited under the `extended`-package rule. **Resolved 2026-09-21: the project owner accepted them as generator output.** Real coverage for those flows comes from our own tests in later chunks.
4. Commit `6e264db` adds the validator subagent, which is tooling rather than a Chunk 0 deliverable. It is a separate commit; the project owner is aware and it stays.

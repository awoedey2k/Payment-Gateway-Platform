...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Wilfredo_Maggio4', email: 'James78@hotmail.com' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Rusty_Schamberger');
      cy.get(`[data-cy="login"]`).should('have.value', 'Rusty_Schamberger');

      cy.get(`[data-cy="firstName"]`).type('Kathlyn');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Kathlyn');

      cy.get(`[data-cy="lastName"]`).type('Williamson');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Williamson');

      cy.get(`[data-cy="email"]`).type('Emmitt.Nader@hotmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Emmitt.Nader@hotmail.com');

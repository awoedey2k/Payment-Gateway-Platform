...
describe('Country e2e test', () => {
  ...
  let username: string;
  let password: string;
  const countrySample = { isoCode: 'ti', name: 'huzzah immediate onto', defaultCurrencyCode: 'whereas boohoo' };
  ...
  it('Countries menu should load Countries page', () => {
    ...
    cy.location('pathname').should('eq', countryPageUrl);
  });
  ...
  describe('Country page', () => {
    describe('create button click', () => {
      ...
      it('should load create Country page', () => {
        ...
        cy.location('pathname').should('eq', `${countryPageUrl}/new`);
  ...
  describe('new Country page', () => {
    ...
    it('should create an instance of Country', () => {
      cy.get(`[data-cy="isoCode"]`).type('na');
      cy.get(`[data-cy="isoCode"]`).should('have.value', 'na');

      cy.get(`[data-cy="name"]`).type('likely majestically');
      cy.get(`[data-cy="name"]`).should('have.value', 'likely majestically');

      cy.get(`[data-cy="defaultCurrencyCode"]`).type('for gnaw joshingly');
      cy.get(`[data-cy="defaultCurrencyCode"]`).should('have.value', 'for gnaw joshingly');

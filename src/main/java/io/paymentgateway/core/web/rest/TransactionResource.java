...
import io.paymentgateway.core.service.TransactionQueryService;
...
/**
 * REST controller for managing {@link io.paymentgateway.core.domain.Transaction}.
 */
@RestController
...
public class TransactionResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new transactionDTO, or with status {@code 400 (Bad Request)} if the transaction already has an ID.
    ...
    public ResponseEntity<TransactionDTO> partialUpdateTransaction(
        ...
        LOG.debug("REST request to partially update Transaction : {}, {}", id, transactionDTO);
        if (transactionDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, transactionDTO.getId())) {

...
import io.paymentgateway.core.service.PaymentMethodQueryService;
import io.paymentgateway.core.service.PaymentMethodService;
...
/**
 * REST controller for managing {@link io.paymentgateway.core.domain.PaymentMethod}.
 */
@RestController
...
public class PaymentMethodResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new paymentMethodDTO, or with status {@code 400 (Bad Request)} if the paymentMethod already has an ID.
    ...
    public ResponseEntity<PaymentMethodDTO> partialUpdatePaymentMethod(
        ...
        LOG.debug("REST request to partially update PaymentMethod : {}, {}", id, paymentMethodDTO);
        if (paymentMethodDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, paymentMethodDTO.getId())) {
    ...
    /**
     * {@code GET  /payment-methods} : get all the Payment Methods.
     *

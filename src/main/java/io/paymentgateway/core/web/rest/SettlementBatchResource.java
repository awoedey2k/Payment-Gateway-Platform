...
import io.paymentgateway.core.service.SettlementBatchQueryService;
...
/**
 * REST controller for managing {@link io.paymentgateway.core.domain.SettlementBatch}.
 */
@RestController
...
public class SettlementBatchResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new settlementBatchDTO, or with status {@code 400 (Bad Request)} if the settlementBatch already has an ID.
    ...
    public ResponseEntity<SettlementBatchDTO> createSettlementBatch(@Valid @RequestBody SettlementBatchDTO settlementBatchDTO)
        ...
        return ResponseEntity.created(new URI("/api/settlement-batches/" + settlementBatchDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, settlementBatchDTO.getId().toString()))
    ...
    public ResponseEntity<SettlementBatchDTO> updateSettlementBatch(
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, settlementBatchDTO.getId().toString()))
            .body(settlementBatchDTO);
    }
    ...
    public ResponseEntity<SettlementBatchDTO> partialUpdateSettlementBatch(
        ...
        LOG.debug("REST request to partially update SettlementBatch : {}, {}", id, settlementBatchDTO);
        if (settlementBatchDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, settlementBatchDTO.getId())) {
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, settlementBatchDTO.getId().toString())
        );
    }

    /**
     * {@code GET  /settlement-batches} : get all the Settlement Batches.
     *
     * @param pageable the pagination information.
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of Settlement Batches in body.
     */
    @GetMapping("")

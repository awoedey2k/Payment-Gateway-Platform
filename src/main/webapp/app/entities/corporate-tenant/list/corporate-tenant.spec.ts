...
import { sampleWithRequiredData } from '../corporate-tenant.test-samples';
import { CorporateTenantService } from '../service/corporate-tenant.service';

import { CorporateTenant } from './corporate-tenant';

vi.useFakeTimers();

describe('CorporateTenant Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CorporateTenant;
  let fixture: ComponentFixture<CorporateTenant>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CorporateTenant);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 10961 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.corporateTenants()[0]).toEqual(expect.objectContaining({ id: 10961 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 10961 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.corporateTenants()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 10961 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to corporateTenantService', () => {
      ...
      vi.spyOn(service, 'getCorporateTenantIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCorporateTenantIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.corporateTenantsParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.corporateTenantsParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

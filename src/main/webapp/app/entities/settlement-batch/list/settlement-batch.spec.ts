...
import { sampleWithRequiredData } from '../settlement-batch.test-samples';

import { SettlementBatch } from './settlement-batch';

vi.useFakeTimers();

describe('SettlementBatch Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: SettlementBatch;
  let fixture: ComponentFixture<SettlementBatch>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(SettlementBatch);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 6598 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.settlementBatches()[0]).toEqual(expect.objectContaining({ id: 6598 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 6598 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.settlementBatches()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 6598 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to settlementBatchService', () => {
      const entity = { id: 6598 };
      vi.spyOn(service, 'getSettlementBatchIdentifier');
      const id = comp.trackId(entity);
      expect(service.getSettlementBatchIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.settlementBatchesParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.settlementBatchesParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

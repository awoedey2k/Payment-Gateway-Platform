...
import { sampleWithRequiredData } from '../dispute-evidence.test-samples';
...
describe('DisputeEvidence Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: DisputeEvidence;
  let fixture: ComponentFixture<DisputeEvidence>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(DisputeEvidence);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 6499 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.disputeEvidences()[0]).toEqual(expect.objectContaining({ id: 6499 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 6499 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.disputeEvidences()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 6499 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to disputeEvidenceService', () => {
      const entity = { id: 6499 };
      vi.spyOn(service, 'getDisputeEvidenceIdentifier');
      const id = comp.trackId(entity);
      expect(service.getDisputeEvidenceIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.disputeEvidencesParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.disputeEvidencesParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

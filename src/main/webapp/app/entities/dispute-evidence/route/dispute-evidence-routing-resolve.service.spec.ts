...
import disputeEvidenceResolve from './dispute-evidence-routing-resolve.service';

describe('DisputeEvidence routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IDisputeEvidence returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          disputeEvidenceResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(disputeEvidenceResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);

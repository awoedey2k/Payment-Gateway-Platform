...
import { LedgerAccountDeleteDialog } from '../delete/ledger-account-delete-dialog';
...
export class LedgerAccount {
  ...
  readonly isLoading = this.ledgerAccountService.ledgerAccountsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.ledgerAccountService.ledgerAccountsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.ledgerAccounts.set(this.fillComponentAttributesFromResponseBody([...this.ledgerAccountService.ledgerAccounts()]));
    });
    effect(() => {
  ...
  trackId = (item: ILedgerAccount): number => this.ledgerAccountService.getLedgerAccountIdentifier(item);

  delete(ledgerAccount: ILedgerAccount): void {
    const modalRef = this.modalService.open(LedgerAccountDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: ILedgerAccount[]): ILedgerAccount[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.ledgerAccountService.ledgerAccountsParams.set(queryObject);
  }

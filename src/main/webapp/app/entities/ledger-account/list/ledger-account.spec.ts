...
import { sampleWithRequiredData } from '../ledger-account.test-samples';
...
describe('LedgerAccount Management Component', () => {
  ...
  let comp: LedgerAccount;
  let fixture: ComponentFixture<LedgerAccount>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(LedgerAccount);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 3298 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.ledgerAccounts()[0]).toEqual(expect.objectContaining({ id: 3298 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 3298 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.ledgerAccounts()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 3298 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to ledgerAccountService', () => {
      ...
      vi.spyOn(service, 'getLedgerAccountIdentifier');
      const id = comp.trackId(entity);
      expect(service.getLedgerAccountIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.ledgerAccountsParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.ledgerAccountsParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

...
import { sampleWithRequiredData } from '../aml-check.test-samples';
...
describe('AmlCheck Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: AmlCheck;
  let fixture: ComponentFixture<AmlCheck>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(AmlCheck);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 28734 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.amlChecks()[0]).toEqual(expect.objectContaining({ id: 28734 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 28734 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.amlChecks()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 28734 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to amlCheckService', () => {
      const entity = { id: 28734 };
      vi.spyOn(service, 'getAmlCheckIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAmlCheckIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.amlChecksParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.amlChecksParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

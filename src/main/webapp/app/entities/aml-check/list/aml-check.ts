...
import { AmlCheckDeleteDialog } from '../delete/aml-check-delete-dialog';
import { AmlCheckService } from '../service/aml-check.service';

@Component({
  selector: 'jhi-aml-check',
  templateUrl: './aml-check.html',
  imports: [
    RouterLink,
    FontAwesomeModule,
    AlertError,
...
export class AmlCheck {
  readonly amlChecks = signal<IAmlCheck[]>([]);
  ...
  readonly isLoading = this.amlCheckService.amlChecksResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.amlCheckService.amlChecksResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.amlChecks.set(this.fillComponentAttributesFromResponseBody([...this.amlCheckService.amlChecks()]));
    });
    effect(() => {
  ...
  trackId = (item: IAmlCheck): number => this.amlCheckService.getAmlCheckIdentifier(item);

  delete(amlCheck: IAmlCheck): void {
    const modalRef = this.modalService.open(AmlCheckDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.amlCheck = amlCheck;
  ...
  protected fillComponentAttributesFromResponseBody(data: IAmlCheck[]): IAmlCheck[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.amlCheckService.amlChecksParams.set(queryObject);
  }

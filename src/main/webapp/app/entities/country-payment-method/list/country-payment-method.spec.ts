...
import { sampleWithRequiredData } from '../country-payment-method.test-samples';
import { CountryPaymentMethodService } from '../service/country-payment-method.service';

import { CountryPaymentMethod } from './country-payment-method';

vi.useFakeTimers();

describe('CountryPaymentMethod Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CountryPaymentMethod;
  let fixture: ComponentFixture<CountryPaymentMethod>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CountryPaymentMethod);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 19642 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.countryPaymentMethods()[0]).toEqual(expect.objectContaining({ id: 19642 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 19642 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.countryPaymentMethods()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 19642 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to countryPaymentMethodService', () => {
      ...
      vi.spyOn(service, 'getCountryPaymentMethodIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCountryPaymentMethodIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.countryPaymentMethodsParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.countryPaymentMethodsParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

...
import countryPaymentMethodResolve from './country-payment-method-routing-resolve.service';

describe('CountryPaymentMethod routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ICountryPaymentMethod returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          countryPaymentMethodResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(countryPaymentMethodResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);

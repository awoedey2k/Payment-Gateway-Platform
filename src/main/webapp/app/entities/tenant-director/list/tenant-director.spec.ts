...
import { sampleWithRequiredData } from '../tenant-director.test-samples';

import { TenantDirector } from './tenant-director';

vi.useFakeTimers();

describe('TenantDirector Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: TenantDirector;
  let fixture: ComponentFixture<TenantDirector>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(TenantDirector);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 7225 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.tenantDirectors()[0]).toEqual(expect.objectContaining({ id: 7225 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 7225 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.tenantDirectors()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 7225 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to tenantDirectorService', () => {
      const entity = { id: 7225 };
      vi.spyOn(service, 'getTenantDirectorIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTenantDirectorIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.tenantDirectorsParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.tenantDirectorsParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

...
import { TenantWalletDeleteDialog } from '../delete/tenant-wallet-delete-dialog';
import { TenantWalletService } from '../service/tenant-wallet.service';
import { ITenantWallet } from '../tenant-wallet.model';

@Component({
  selector: 'jhi-tenant-wallet',
  templateUrl: './tenant-wallet.html',
...
export class TenantWallet {
  readonly tenantWallets = signal<ITenantWallet[]>([]);
  ...
  readonly isLoading = this.tenantWalletService.tenantWalletsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.tenantWalletService.tenantWalletsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.tenantWallets.set(this.fillComponentAttributesFromResponseBody([...this.tenantWalletService.tenantWallets()]));
    });
    effect(() => {
  ...
  trackId = (item: ITenantWallet): number => this.tenantWalletService.getTenantWalletIdentifier(item);

  delete(tenantWallet: ITenantWallet): void {
    const modalRef = this.modalService.open(TenantWalletDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.tenantWallet = tenantWallet;
  ...
  protected fillComponentAttributesFromResponseBody(data: ITenantWallet[]): ITenantWallet[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.tenantWalletService.tenantWalletsParams.set(queryObject);
  }

...
import { sampleWithRequiredData } from '../tenant-wallet.test-samples';

import { TenantWallet } from './tenant-wallet';

vi.useFakeTimers();

describe('TenantWallet Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: TenantWallet;
  let fixture: ComponentFixture<TenantWallet>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(TenantWallet);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 17782 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.tenantWallets()[0]).toEqual(expect.objectContaining({ id: 17782 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 17782 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.tenantWallets()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 17782 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to tenantWalletService', () => {
      const entity = { id: 17782 };
      vi.spyOn(service, 'getTenantWalletIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTenantWalletIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.tenantWalletsParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.tenantWalletsParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

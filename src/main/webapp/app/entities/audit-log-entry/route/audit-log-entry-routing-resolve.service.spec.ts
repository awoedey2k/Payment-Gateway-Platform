...
import auditLogEntryResolve from './audit-log-entry-routing-resolve.service';

describe('AuditLogEntry routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IAuditLogEntry returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          auditLogEntryResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(auditLogEntryResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);

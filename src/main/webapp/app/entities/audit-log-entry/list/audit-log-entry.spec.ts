...
import { sampleWithRequiredData } from '../audit-log-entry.test-samples';
...
describe('AuditLogEntry Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: AuditLogEntry;
  let fixture: ComponentFixture<AuditLogEntry>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(AuditLogEntry);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 27321 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.auditLogEntries()[0]).toEqual(expect.objectContaining({ id: 27321 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 27321 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.auditLogEntries()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 27321 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to auditLogEntryService', () => {
      const entity = { id: 27321 };
      vi.spyOn(service, 'getAuditLogEntryIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAuditLogEntryIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.auditLogEntriesParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.auditLogEntriesParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

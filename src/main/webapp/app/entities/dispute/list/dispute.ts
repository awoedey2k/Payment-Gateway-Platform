...
export class Dispute {
  readonly disputes = signal<IDispute[]>([]);
  ...
  readonly isLoading = this.disputeService.disputesResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.disputeService.disputesResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.disputes.set(this.fillComponentAttributesFromResponseBody([...this.disputeService.disputes()]));
    });
    effect(() => {
  ...
  trackId = (item: IDispute): number => this.disputeService.getDisputeIdentifier(item);

  delete(dispute: IDispute): void {
    const modalRef = this.modalService.open(DisputeDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IDispute[]): IDispute[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.disputeService.disputesParams.set(queryObject);
  }

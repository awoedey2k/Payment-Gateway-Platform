...
import payoutScheduleResolve from './payout-schedule-routing-resolve.service';

describe('PayoutSchedule routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IPayoutSchedule returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          payoutScheduleResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(payoutScheduleResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);

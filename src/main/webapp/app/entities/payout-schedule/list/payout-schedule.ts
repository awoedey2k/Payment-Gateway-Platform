...
import { PayoutScheduleDeleteDialog } from '../delete/payout-schedule-delete-dialog';
import { IPayoutSchedule } from '../payout-schedule.model';
import { PayoutScheduleService } from '../service/payout-schedule.service';

@Component({
  selector: 'jhi-payout-schedule',
  templateUrl: './payout-schedule.html',
...
export class PayoutSchedule {
  readonly payoutSchedules = signal<IPayoutSchedule[]>([]);
  ...
  readonly isLoading = this.payoutScheduleService.payoutSchedulesResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.payoutScheduleService.payoutSchedulesResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.payoutSchedules.set(this.fillComponentAttributesFromResponseBody([...this.payoutScheduleService.payoutSchedules()]));
    });
    effect(() => {
  ...
  trackId = (item: IPayoutSchedule): number => this.payoutScheduleService.getPayoutScheduleIdentifier(item);

  delete(payoutSchedule: IPayoutSchedule): void {
    const modalRef = this.modalService.open(PayoutScheduleDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.payoutSchedule = payoutSchedule;
  ...
  protected fillComponentAttributesFromResponseBody(data: IPayoutSchedule[]): IPayoutSchedule[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.payoutScheduleService.payoutSchedulesParams.set(queryObject);
  }

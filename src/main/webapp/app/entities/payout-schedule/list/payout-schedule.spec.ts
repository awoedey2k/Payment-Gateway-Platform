...
import { sampleWithRequiredData } from '../payout-schedule.test-samples';
...
describe('PayoutSchedule Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: PayoutSchedule;
  let fixture: ComponentFixture<PayoutSchedule>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(PayoutSchedule);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 4713 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.payoutSchedules()[0]).toEqual(expect.objectContaining({ id: 4713 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 4713 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.payoutSchedules()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 4713 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to payoutScheduleService', () => {
      const entity = { id: 4713 };
      vi.spyOn(service, 'getPayoutScheduleIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPayoutScheduleIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.payoutSchedulesParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.payoutSchedulesParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

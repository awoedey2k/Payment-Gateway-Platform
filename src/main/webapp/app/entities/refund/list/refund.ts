...
import { RefundDeleteDialog } from '../delete/refund-delete-dialog';
...
export class Refund {
  readonly refunds = signal<IRefund[]>([]);
  ...
  protected readonly refundService = inject(RefundService);
  // eslint-disable-next-line @typescript-eslint/member-ordering
  readonly isLoading = this.refundService.refundsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.refundService.refundsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.refunds.set(this.fillComponentAttributesFromResponseBody([...this.refundService.refunds()]));
    });
    effect(() => {
  ...
  trackId = (item: IRefund): number => this.refundService.getRefundIdentifier(item);

  delete(refund: IRefund): void {
    const modalRef = this.modalService.open(RefundDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IRefund[]): IRefund[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.refundService.refundsParams.set(queryObject);
  }

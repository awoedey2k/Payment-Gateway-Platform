...
import { TransactionDeleteDialog } from '../delete/transaction-delete-dialog';
...
export class Transaction {
  ...
  readonly isLoading = this.transactionService.transactionsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.transactionService.transactionsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.transactions.set(this.fillComponentAttributesFromResponseBody([...this.transactionService.transactions()]));
    });
    effect(() => {
  ...
  delete(transaction: ITransaction): void {
    const modalRef = this.modalService.open(TransactionDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: ITransaction[]): ITransaction[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.transactionService.transactionsParams.set(queryObject);
  }

...
import { sampleWithRequiredData } from '../journal-line.test-samples';
...
describe('JournalLine Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: JournalLine;
  let fixture: ComponentFixture<JournalLine>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(JournalLine);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 13943 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.journalLines()[0]).toEqual(expect.objectContaining({ id: 13943 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 13943 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.journalLines()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 13943 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to journalLineService', () => {
      const entity = { id: 13943 };
      vi.spyOn(service, 'getJournalLineIdentifier');
      const id = comp.trackId(entity);
      expect(service.getJournalLineIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.journalLinesParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.journalLinesParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

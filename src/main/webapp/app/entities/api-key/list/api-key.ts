...
import { ApiKeyDeleteDialog } from '../delete/api-key-delete-dialog';
...
export class ApiKey {
  readonly apiKeys = signal<IApiKey[]>([]);
  ...
  readonly isLoading = this.apiKeyService.apiKeysResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.apiKeyService.apiKeysResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.apiKeys.set(this.fillComponentAttributesFromResponseBody([...this.apiKeyService.apiKeys()]));
    });
    effect(() => {
  ...
  trackId = (item: IApiKey): number => this.apiKeyService.getApiKeyIdentifier(item);

  delete(apiKey: IApiKey): void {
    const modalRef = this.modalService.open(ApiKeyDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IApiKey[]): IApiKey[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.apiKeyService.apiKeysParams.set(queryObject);
  }

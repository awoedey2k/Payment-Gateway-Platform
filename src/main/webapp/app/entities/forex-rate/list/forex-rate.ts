...
import { ForexRateDeleteDialog } from '../delete/forex-rate-delete-dialog';
import { IForexRate } from '../forex-rate.model';
import { ForexRateService } from '../service/forex-rate.service';

@Component({
  selector: 'jhi-forex-rate',
  templateUrl: './forex-rate.html',
  imports: [
    RouterLink,
    FontAwesomeModule,
    AlertError,
...
export class ForexRate {
  readonly forexRates = signal<IForexRate[]>([]);
  ...
  readonly isLoading = this.forexRateService.forexRatesResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.forexRateService.forexRatesResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.forexRates.set(this.fillComponentAttributesFromResponseBody([...this.forexRateService.forexRates()]));
    });
    effect(() => {
  ...
  trackId = (item: IForexRate): number => this.forexRateService.getForexRateIdentifier(item);

  delete(forexRate: IForexRate): void {
    const modalRef = this.modalService.open(ForexRateDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.forexRate = forexRate;
  ...
  protected fillComponentAttributesFromResponseBody(data: IForexRate[]): IForexRate[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.forexRateService.forexRatesParams.set(queryObject);
  }

...
import { sampleWithRequiredData } from '../forex-rate.test-samples';
...
describe('ForexRate Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: ForexRate;
  let fixture: ComponentFixture<ForexRate>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(ForexRate);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 29998 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.forexRates()[0]).toEqual(expect.objectContaining({ id: 29998 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 29998 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.forexRates()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 29998 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to forexRateService', () => {
      const entity = { id: 29998 };
      vi.spyOn(service, 'getForexRateIdentifier');
      const id = comp.trackId(entity);
      expect(service.getForexRateIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.forexRatesParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.forexRatesParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

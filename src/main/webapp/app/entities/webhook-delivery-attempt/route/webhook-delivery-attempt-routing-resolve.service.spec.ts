...
import webhookDeliveryAttemptResolve from './webhook-delivery-attempt-routing-resolve.service';

describe('WebhookDeliveryAttempt routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IWebhookDeliveryAttempt returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          webhookDeliveryAttemptResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(webhookDeliveryAttemptResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);

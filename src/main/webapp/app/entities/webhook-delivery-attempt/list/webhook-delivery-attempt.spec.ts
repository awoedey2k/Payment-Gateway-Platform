...
import { sampleWithRequiredData } from '../webhook-delivery-attempt.test-samples';

import { WebhookDeliveryAttempt } from './webhook-delivery-attempt';

vi.useFakeTimers();

describe('WebhookDeliveryAttempt Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: WebhookDeliveryAttempt;
  let fixture: ComponentFixture<WebhookDeliveryAttempt>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(WebhookDeliveryAttempt);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 3237 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.webhookDeliveryAttempts()[0]).toEqual(expect.objectContaining({ id: 3237 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 3237 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.webhookDeliveryAttempts()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 3237 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to webhookDeliveryAttemptService', () => {
      const entity = { id: 3237 };
      vi.spyOn(service, 'getWebhookDeliveryAttemptIdentifier');
      const id = comp.trackId(entity);
      expect(service.getWebhookDeliveryAttemptIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.webhookDeliveryAttemptsParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.webhookDeliveryAttemptsParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

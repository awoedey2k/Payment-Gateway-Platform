...
import { sampleWithRequiredData } from '../tenant-fee-config.test-samples';

import { TenantFeeConfig } from './tenant-fee-config';

vi.useFakeTimers();

describe('TenantFeeConfig Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: TenantFeeConfig;
  let fixture: ComponentFixture<TenantFeeConfig>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(TenantFeeConfig);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 22814 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.tenantFeeConfigs()[0]).toEqual(expect.objectContaining({ id: 22814 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 22814 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.tenantFeeConfigs()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 22814 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to tenantFeeConfigService', () => {
      const entity = { id: 22814 };
      vi.spyOn(service, 'getTenantFeeConfigIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTenantFeeConfigIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.tenantFeeConfigsParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.tenantFeeConfigsParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

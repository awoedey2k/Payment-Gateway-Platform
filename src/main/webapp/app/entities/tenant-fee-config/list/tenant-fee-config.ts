...
export class TenantFeeConfig {
  ...
  readonly isLoading = this.tenantFeeConfigService.tenantFeeConfigsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.tenantFeeConfigService.tenantFeeConfigsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.tenantFeeConfigs.set(this.fillComponentAttributesFromResponseBody([...this.tenantFeeConfigService.tenantFeeConfigs()]));
    });
    effect(() => {
  ...
  trackId = (item: ITenantFeeConfig): number => this.tenantFeeConfigService.getTenantFeeConfigIdentifier(item);

  delete(tenantFeeConfig: ITenantFeeConfig): void {
    const modalRef = this.modalService.open(TenantFeeConfigDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.tenantFeeConfig = tenantFeeConfig;
  ...
  protected fillComponentAttributesFromResponseBody(data: ITenantFeeConfig[]): ITenantFeeConfig[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.tenantFeeConfigService.tenantFeeConfigsParams.set(queryObject);
  }

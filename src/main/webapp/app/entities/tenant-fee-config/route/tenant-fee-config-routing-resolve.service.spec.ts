...
import tenantFeeConfigResolve from './tenant-fee-config-routing-resolve.service';

describe('TenantFeeConfig routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ITenantFeeConfig returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          tenantFeeConfigResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(tenantFeeConfigResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);

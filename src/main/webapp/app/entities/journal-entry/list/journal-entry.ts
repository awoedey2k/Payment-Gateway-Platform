...
export class JournalEntry {
  readonly journalEntries = signal<IJournalEntry[]>([]);
  ...
  readonly isLoading = this.journalEntryService.journalEntriesResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.journalEntryService.journalEntriesResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.journalEntries.set(this.fillComponentAttributesFromResponseBody([...this.journalEntryService.journalEntries()]));
    });
    effect(() => {
  ...
  trackId = (item: IJournalEntry): number => this.journalEntryService.getJournalEntryIdentifier(item);

  delete(journalEntry: IJournalEntry): void {
    const modalRef = this.modalService.open(JournalEntryDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.journalEntry = journalEntry;
  ...
  protected fillComponentAttributesFromResponseBody(data: IJournalEntry[]): IJournalEntry[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.journalEntryService.journalEntriesParams.set(queryObject);
  }

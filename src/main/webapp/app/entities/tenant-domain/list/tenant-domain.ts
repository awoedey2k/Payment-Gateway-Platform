...
import { TenantDomainDeleteDialog } from '../delete/tenant-domain-delete-dialog';
import { TenantDomainService } from '../service/tenant-domain.service';
import { ITenantDomain } from '../tenant-domain.model';

@Component({
  selector: 'jhi-tenant-domain',
  templateUrl: './tenant-domain.html',
...
export class TenantDomain {
  readonly tenantDomains = signal<ITenantDomain[]>([]);
  ...
  readonly isLoading = this.tenantDomainService.tenantDomainsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.tenantDomainService.tenantDomainsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.tenantDomains.set(this.fillComponentAttributesFromResponseBody([...this.tenantDomainService.tenantDomains()]));
    });
    effect(() => {
  ...
  trackId = (item: ITenantDomain): number => this.tenantDomainService.getTenantDomainIdentifier(item);

  delete(tenantDomain: ITenantDomain): void {
    const modalRef = this.modalService.open(TenantDomainDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.tenantDomain = tenantDomain;
  ...
  protected fillComponentAttributesFromResponseBody(data: ITenantDomain[]): ITenantDomain[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.tenantDomainService.tenantDomainsParams.set(queryObject);
  }

...
import { sampleWithRequiredData } from '../tenant-domain.test-samples';

import { TenantDomain } from './tenant-domain';

vi.useFakeTimers();

describe('TenantDomain Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: TenantDomain;
  let fixture: ComponentFixture<TenantDomain>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(TenantDomain);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 19717 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.tenantDomains()[0]).toEqual(expect.objectContaining({ id: 19717 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 19717 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.tenantDomains()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 19717 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to tenantDomainService', () => {
      const entity = { id: 19717 };
      vi.spyOn(service, 'getTenantDomainIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTenantDomainIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.tenantDomainsParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.tenantDomainsParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

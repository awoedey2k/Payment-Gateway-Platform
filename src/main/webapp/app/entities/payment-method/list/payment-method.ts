...
import { PaymentMethodDeleteDialog } from '../delete/payment-method-delete-dialog';
...
export class PaymentMethod {
  ...
  readonly isLoading = this.paymentMethodService.paymentMethodsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.paymentMethodService.paymentMethodsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.paymentMethods.set(this.fillComponentAttributesFromResponseBody([...this.paymentMethodService.paymentMethods()]));
    });
    effect(() => {
  ...
  delete(paymentMethod: IPaymentMethod): void {
    const modalRef = this.modalService.open(PaymentMethodDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IPaymentMethod[]): IPaymentMethod[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.paymentMethodService.paymentMethodsParams.set(queryObject);
  }

...
import { sampleWithRequiredData } from '../routing-rule.test-samples';
...
describe('RoutingRule Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: RoutingRule;
  let fixture: ComponentFixture<RoutingRule>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(RoutingRule);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 9786 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.routingRules()[0]).toEqual(expect.objectContaining({ id: 9786 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 9786 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.routingRules()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 9786 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to routingRuleService', () => {
      const entity = { id: 9786 };
      vi.spyOn(service, 'getRoutingRuleIdentifier');
      const id = comp.trackId(entity);
      expect(service.getRoutingRuleIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.routingRulesParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.routingRulesParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });

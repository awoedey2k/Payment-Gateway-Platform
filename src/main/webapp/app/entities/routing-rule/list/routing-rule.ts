...
export class RoutingRule {
  ...
  readonly isLoading = this.routingRuleService.routingRulesResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.routingRuleService.routingRulesResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.routingRules.set(this.fillComponentAttributesFromResponseBody([...this.routingRuleService.routingRules()]));
    });
    effect(() => {
  ...
  trackId = (item: IRoutingRule): number => this.routingRuleService.getRoutingRuleIdentifier(item);

  delete(routingRule: IRoutingRule): void {
    const modalRef = this.modalService.open(RoutingRuleDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.routingRule = routingRule;
  ...
  protected fillComponentAttributesFromResponseBody(data: IRoutingRule[]): IRoutingRule[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.routingRuleService.routingRulesParams.set(queryObject);
  }
